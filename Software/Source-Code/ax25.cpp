...
uint8_t AFSK_tone_adj_space = APRS_2400_SPACE_DELAY;
uint16_t AFSK_tone_adj_mark = APRS_1200_MARK_DELAY;
...
static void ax25_set_rectangle_wave_out(bool rectangle_wave_out_state)
{
  ...
  {
    SX1278_mod_direct_out(AFSK_tone_adj_mark); // Send 1200Hz mark for ~833us, so theoretical a delay between state changes of ~417us
  }
  else
  {
    SX1278_mod_direct_out(AFSK_tone_adj_space); // Send first 2400Hz space for ~417us, so theoretical a delay between state changes of ~208s
    SX1278_mod_direct_out(AFSK_tone_adj_space); // Send second 2400Hz space for ~417us, so theoretical a delay between state changes of ~208us
  }
}
