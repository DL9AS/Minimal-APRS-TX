...
void SX1278_mod_direct_out(uint32_t delay)
{
    ...
    digitalWriteFast(SX1278_DIO2, HIGH);
    delayMicroseconds(delay);
    digitalWriteFast(SX1278_DIO2, LOW);
    delayMicroseconds(delay);
...
void SX1278_set_TX_frequency(uint64_t *freq)
{
  ...
  // Freg = (Frf * 2^19) / Fxo
  // Resolution is 61.035 Hz if Fxo = 32 MHz
  uint64_t freq_tmp = (((*freq + SX1278_FREQUENCY_CORRECTION) << 19) / SX1278_CRYSTAL_FREQ);
...
// This measurement must be performed with SX1278 in Fsk RX/TX mode!
void SX1278_measure_internal_temperature(int16_t *temp)
{
